...
const MISIONES_DEFAULT = [
  { id:"radio-am", title:"Radio AM", icon:"📻", color:"#f97316", glow:"rgba(249,115,22,0.35)", description:"Construye tu propio receptor de Radio AM con materiales básicos",
    retos:[{id:1,stars:1,title:"¿Qué es una onda?",desc:"Comprende amplitud, frecuencia y longitud de onda de las ondas electromagnéticas."},{id:2,stars:1,title:"El espectro de radio",desc:"Explora las frecuencias AM (530–1700 kHz) y cómo se diferencia de FM."},{id:3,stars:2,title:"Componentes del receptor",desc:"Identifica: diodo 1N34A, condensador variable, bobina de ferrita, auricular alta impedancia."},{id:4,stars:2,title:"La bobina artesanal",desc:"Enrolla 60 vueltas de alambre de cobre esmaltado en núcleo de ferrita."},{id:5,stars:3,title:"El detector de envolvente",desc:"El diodo rectifica la portadora y extrae el audio."},{id:6,stars:3,title:"¡Arma tu Radio AM!",desc:"Integra todo, conecta antena de 1m y sintoniza una emisora de Medellín."}]},
  { id:"transmisor-fm", title:"Transmisor FM", icon:"📡", color:"#eab308", glow:"rgba(234,179,8,0.35)", description:"Diseña un transmisor FM de bajo alcance con componentes accesibles",
    retos:[{id:1,stars:1,title:"¿Cómo viaja tu voz?",desc:"Comprende la modulación FM."},{id:2,stars:2,title:"El oscilador LC",desc:"Bobina + condensador generan la portadora."},{id:3,stars:2,title:"Transistor amplificador",desc:"Usa el BC547 como amplificador."},{id:4,stars:2,title:"La antena irradiadora",desc:"Calcula longitud λ/4 para tu frecuencia."},{id:5,stars:3,title:"¡Transmite tu señal!",desc:"Sintoniza entre 88–108 MHz y escúchate en un radio FM."}]},
  { id:"brazo-robotico", title:"Brazo Robótico", icon:"🦾", color:"#22c55e", glow:"rgba(34,197,94,0.35)", description:"Programa y construye un brazo robótico con Arduino UNO y piezas impresas en 3D",
    retos:[{id:1,stars:1,title:"Conoce tu Arduino UNO",desc:"Pines digitales, analógicos, PWM. Carga Blink."},{id:2,stars:1,title:"El servomotor SG90",desc:"Programa ángulos con Servo.h de 0° a 180°."},{id:3,stars:2,title:"Control por potenciómetro",desc:"Lee con analogRead() y mapea a ángulos del servo."},{id:4,stars:2,title:"Diseño 3D en TinkerCAD",desc:"Diseña: base, hombro, codo, muñeca y pinza."},{id:5,stars:2,title:"Impresión y ensamble",desc:"Cura: capa 0.2mm, relleno 20%."},{id:6,stars:3,title:"Secuencia de movimientos",desc:"Programa el brazo para recoger y depositar objetos."},{id:7,stars:3,title:"¡Control Bluetooth!",desc:"Agrega HC-05 y controla desde el celular."}]},
];
...
PERSONALIDAD: Animado, motivador, como guía de aventuras. Usas emojis con moderación.
NUNCA das respuestas directas: guías con pistas. Si se acercan: "¡Vas por buen camino! 🔥"
METODOLOGÍA: 1) Pregunta qué sabe, 2) Una pista a la vez, 3) "¿Qué pasaría si...?", 4) Celebra logros: "¡+20 puntos de maestría! ⭐"
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily:"'Syne','Inter',sans-serif", background:C.bg, color:C.text, height:"100vh", overflow:"hidden", position:"relative" }}>
      ...
      {view==="teacher" && <TeacherView user={user} onLogout={logout} misiones={misiones} setMisiones={setMisiones} />}
    ...
    <div style={{ display:"flex", height:"100vh", position:"relative", zIndex:5 }}>
      <div style={{ flex:1, background:`linear-gradient(135deg,#070d1a,#0d1f3c)`, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:40, borderRight:`1px solid ${C.border}` }}>
        ...
        <div style={{ marginTop:48, background:`${C.accent}10`, border:`1px solid ${C.border}`, borderRadius:14, padding:20, width:"100%", maxWidth:280 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <div><div style={{ fontSize:13, fontWeight:700 }}>Fabio Alberto Ortiz M.</div><div style={{ fontSize:11, color:C.accent, marginTop:2 }}>fabioortiz37422@sabaneta.edu.co</div></div>
          </div>
        </div>
      </div>
      <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", padding:40 }}>
        <div style={{ width:"100%", maxWidth:420 }}>
          ...
          {mode==="teacher" && (
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              <div style={{ textAlign:"center" }}><div style={{ fontSize:18, fontWeight:800 }}>📚 Ingreso Docentes</div><div style={{ fontSize:12, color:C.muted, marginTop:4 }}>Correo y contraseña institucional</div></div>
              ...
              {error && <div style={{ background:"#ff444422", border:"1px solid #ff444444", color:"#ff7777", padding:"10px 14px", borderRadius:8, fontSize:13 }}>{error}</div>}
              <button style={{ padding:"13px 20px", background:`linear-gradient(135deg,${C.accent},${C.accent2})`, border:"none", borderRadius:12, color:"#fff", fontWeight:700, fontSize:14, cursor:"pointer" }} onClick={handleSubmit} disabled={!email||!pw}>Ingresar al sistema ➤</button>
            </div>
          )}
        </div>
      </div>
...
// ═══════════════════════════════════════════════════════════════
// PANEL DE PROGRESO COMPARTIDO (Admin + Teacher usan el mismo)
// ═══════════════════════════════════════════════════════════════
function ProgresoPanel({ misiones }) {
  ...
  const excelRows = estudiantes.map((e,i) => ({
    Posición: i+1, Nombre: e.nombre_estudiante, Grado: e.grado, Grupo: e.grupo||"—",
    XP_Total: e.xp_total, Nivel: e.nivel||1, Misión: e.mision_id||"libre"
  }));
        ...
        <>
              ...
              return (
                <div key={g} style={{ marginBottom:12 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                    ...
                    <span style={{ fontSize:11, color:C.muted }}>{d.count} estudiantes · {d.xp} XP</span>
                  </div>
          ...
          <Card title="🎓 Detalle por Estudiante">
            ...
            <div style={{ display:"flex", gap:10, marginBottom:14, alignItems:"center", flexWrap:"wrap" }}>
              ...
              <button onClick={()=>setOrdenAZ(!ordenAZ)} style={{ padding:"6px 12px", background:ordenAZ?`${C.accent2}33`:`${C.border}`, border:`1px solid ${ordenAZ?C.accent2:C.border}`, borderRadius:8, color:ordenAZ?C.accent2:C.muted, fontSize:12, cursor:"pointer" }}>
                {ordenAZ ? "🔤 A→Z" : "🏆 Mayor XP"}
              </button>
              <button onClick={()=>downloadExcel(excelRows, `nexus_progreso_${filtroGrado}_${filtroGrupo}`)} style={{ padding:"6px 12px", background:`${C.accent3}22`, border:`1px solid ${C.accent3}44`, borderRadius:8, color:C.accent3, fontSize:12, cursor:"pointer", marginLeft:"auto" }}>
                ⬇️ Descargar Excel
              </button>
            </div>

            <div style={{ fontSize:11, color:C.muted, marginBottom:10 }}>
              {estudiantes.length} estudiante{estudiantes.length!==1?"s":""} {filtroGrado!=="todos"?`· Grado ${filtroGrado}`:""} {filtroGrupo!=="todos"?`· Grupo ${filtroGrupo}`:""}
            </div>
            ...
            {estudiantes.length > 0 ? estudiantes.map((e,i)=>(
              <div key={i} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 14px", background:C.surface, borderRadius:10, border:`1px solid ${C.border}`, marginBottom:6 }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:11, color:C.muted }}>Grado {e.grado} · Grupo {e.grupo||"—"} · Nivel {e.nivel||1}</div>
                </div>
                {e.mision_id && <span style={{ padding:"3px 8px", borderRadius:6, fontSize:10, background:`${C.accent}22`, color:C.accent }}>{e.mision_id}</span>}
                <span style={{ fontFamily:"'Orbitron',monospace", color:C.accent3, fontWeight:700, fontSize:13 }}>{e.xp_total} XP</span>
              </div>
            )) : <div style={{ color:C.muted, fontSize:12 }}>Sin estudiantes con los filtros seleccionados.</div>}
          </Card>
        </>
      )}
      {!loading && stats?.error && (
        <InfoBox title="⚠️ Error"><Row k="Detalle" v={stats.error} /></InfoBox>
      )}
    </Page>
  );
}
...
// ═══════════════════════════════════════════════════════════════
// DASHBOARD COMPARTIDO (Admin + Teacher)
// ═══════════════════════════════════════════════════════════════
function DashboardPanel({ user, misiones }) {
  ...
  let topEstudiantes = stats?.topEstudiantes || [];
  if (filtroGrado !== "todos") topEstudiantes = topEstudiantes.filter(e => e.grado === filtroGrado);
  if (filtroGrupo !== "todos") topEstudiantes = topEstudiantes.filter(e => e.grupo === filtroGrupo);
  if (ordenAZ) topEstudiantes = [...topEstudiantes].sort((a,b) => a.nombre_estudiante.localeCompare(b.nombre_estudiante));

  const excelRows = topEstudiantes.map((e,i) => ({
    Posición: i+1, Nombre: e.nombre_estudiante, Grado: e.grado, Grupo: e.grupo||"—", XP_Total: e.xp_total, Nivel: e.nivel||1, Misión: e.mision_id||"libre"
  }));

  const isAdmin = user?.role === "admin";

  return (
    <Page title={isAdmin ? "Panel de Administración" : "📊 Mi Panel Docente"} desc={`Bienvenido, ${user.name}.`}>
      ...
      <div style={grid4}>
        {[
          ...
          isAdmin ? ["📚","Docentes", stats?.resumen?.totalDocentes ?? "—", C.accent2] : ["📚","Mi asignatura", user.subject||"—", C.accent2],
      ...
      <Card title="🏆 Top Estudiantes">
        <div style={{ display:"flex", gap:10, marginBottom:14, alignItems:"center", flexWrap:"wrap" }}>
          ...
          <button onClick={()=>downloadExcel(excelRows, `nexus_top_${filtroGrado}_${filtroGrupo}`)} style={{ padding:"6px 12px", background:`${C.accent3}22`, border:`1px solid ${C.accent3}44`, borderRadius:8, color:C.accent3, fontSize:12, cursor:"pointer", marginLeft:"auto" }}>
            ⬇️ Descargar Excel
          </button>
        </div>

        <div style={{ fontSize:11, color:C.muted, marginBottom:10 }}>
          {topEstudiantes.length} resultado{topEstudiantes.length!==1?"s":""}
        </div>

        {topEstudiantes.length > 0 ? topEstudiantes.slice(0,10).map((e,i)=>(
          <div key={i} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 12px", background:C.surface, borderRadius:10, marginBottom:6, border:`1px solid ${C.border}` }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:10, color:C.muted }}>Grado {e.grado} · Grupo {e.grupo||"—"}</div>
            </div>
        ...
        )) : <div style={{ color:C.muted, fontSize:12 }}>Sin actividad registrada con este filtro.</div>}
      </Card>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
        ...
        <Card title="🕐 Actividad reciente">
          {stats?.actividadReciente?.length > 0
            ? stats.actividadReciente.slice(0,6).map((a,i)=>(
              <div key={i} style={{ display:"flex", alignItems:"center", gap:10, padding:"7px 0", borderBottom:`1px solid ${C.border}` }}>
                <span style={{ fontSize:14 }}>🎓</span>
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:11, fontWeight:600 }}>{a.nombre_estudiante}</div>
                  <div style={{ fontSize:10, color:C.muted }}>Grado {a.grado} · {new Date(a.updated_at).toLocaleDateString("es-CO")}</div>
                </div>
...
// ═══════════════════════════════════════════════════════════════
// GESTIÓN DE MISIONES (compartida Admin + Teacher)
// ═══════════════════════════════════════════════════════════════
function MisionesPanel({ misiones, setMisiones }) {
  ...
  const iniciarNueva = () => { setForm({ id:`mision-${Date.now()}`, title:"", icon:"📻", color:"#f97316", description:"", retos:[] }); setEditando("nueva"); };
  ...
  return (
    <Page title={editando==="nueva"?"➕ Nueva Misión":"✏️ Editar Misión"}>
      ...
      <Card title="📋 Información general">
        ...
        <div><div style={lbl}>Color</div><div style={{ display:"flex", gap:8, flexWrap:"wrap", marginTop:4 }}>{COLORES_MISION.map(col=><button key={col} onClick={()=>setForm(p=>({...p,color:col}))} style={{ width:32, height:32, borderRadius:"50%", background:col, border:`3px solid ${form.color===col?"#fff":col}`, cursor:"pointer", transform:form.color===col?"scale(1.2)":"scale(1)", transition:"transform .15s" }} />)}</div></div>
      </Card>
      <Card title="⭐ Retos">
        {form.retos.map((r,i)=>(
...
function AdminView({ user, onLogout, misiones, setMisiones }) {
  ...
  return (
    <Layout sidebar={<Sidebar user={user} onLogout={onLogout} tab={tab} setTab={setTab} tabs={[
      ...
      {tab==="progreso"  && <ProgresoPanel misiones={misiones} />}
      {tab==="missions"  && <MisionesPanel misiones={misiones} setMisiones={setMisiones} />}
      {tab==="users"     && <AdminUsuarios />}
      {tab==="subjects"  && <Page title="Asignaturas">{["Tecnología e Informática · Prof. Fabio Ortiz","Matemáticas · Por asignar","Ciencias Naturales · Por asignar"].map((s,i)=><Card key={i} title={s}><div style={{ fontSize:13, color:C.muted }}>NEXUS activo</div></Card>)}</Page>}
    </Layout>
  );
}

// ═══════════════════════════════════════════════════════════════
// TEACHER VIEW — mismo dashboard, progreso y misiones que admin
// ═══════════════════════════════════════════════════════════════
function TeacherView({ user, onLogout, misiones, setMisiones }) {
  const [tab, setTab] = useState("dashboard");
  ...
  return (
    <Layout sidebar={<Sidebar user={user} onLogout={onLogout} tab={tab} setTab={setTab} tabs={[
      ...
      {tab==="missions"  && <MisionesPanel misiones={misiones} setMisiones={setMisiones} />}
      {tab==="config" && (
        <Page title="⚙️ Configura NEXUS para tu asignatura">
          ...
          <Card title="📋 Temáticas">
            ...
            <select style={inp} value={cfg.tone} onChange={e=>setCfg({...cfg,tone:e.target.value})}>
              ...
              <option value="gamificado">Gamificado extremo</option>
            </select>
          </Card>
...
function AdminUsuarios() {
  const [nu, setNu] = useState({ name:"", email:"", role:"student", grade:"" });
  const [saved, setSaved] = useState(false);
  return (
      ...
      <Card title="➕ Agregar usuario">
        <div style={grid2}>
          ...
          <select style={inp} value={nu.role} onChange={e=>setNu({...nu,role:e.target.value})}>
            <option value="student">Estudiante</option><option value="teacher">Docente</option><option value="admin">Admin</option>
          </select>
          {nu.role==="student" && <input style={inp} placeholder="Grado" value={nu.grade} onChange={e=>setNu({...nu,grade:e.target.value})} />}
        </div>
        <Btn onClick={()=>{setSaved(true);setTimeout(()=>setSaved(false),2000)}}>{saved?"✅ ¡Guardado!":"Agregar usuario"}</Btn>
      </Card>
      <InfoBox title="💡 Gestión en Supabase">
        <Row k="Tablas" v="docentes / estudiantes" />
        <Row k="Contraseñas" v="bcrypt-generator.com (cost 10)" />
      </InfoBox>
    </Page>
  );
}
...
function StudentView({ user, onLogout, misiones }) {
  ...
  return (
    <Layout sidebar={<Sidebar user={user} onLogout={onLogout} tab={tab} setTab={setTab} tabs={[
        ...
        <div style={{ height:"100%", display:"flex", flexDirection:"column", overflow:"hidden" }}>
          <div style={{ padding:"16px 24px 0", flexShrink:0 }}>
            ...
            <div style={{ display:"flex", gap:8, marginBottom:8 }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:6, background:`${C.accent3}15`, border:`1px solid ${C.accent3}44`, borderRadius:10, padding:"7px 12px", fontSize:11, color:C.accent3 }}>💬 Modo libre activo</div>
            </div>
          </div>
      ...
      {tab==="missions" && <Page title="🗺️ Mapa de Misiones 2025"><MissionMap misiones={misiones} onSelect={id=>{ setMission(id); setTab("chat"); }} /></Page>}
...
function NexusChat({ prompt, userName, compact, user, misionId }) {
  const [msgs, setMsgs] = useState([{ role:"assistant", content:`¡Bienvenido${userName?`, ${userName.split(" ")[0]}`:""}! 🚀 Soy **NEXUS**. No te daré respuestas directas... ¡eso sería aburrido! Te guío con pistas para que TÚ descubras el conocimiento.\n\n💬 **Modo libre:** pregunta lo que quieras sobre tecnología.\n🗺️ **O elige una misión** en el menú lateral. ¡Tú decides! 🎯` }]);
  ...
  const send = async txt => {
    ...
    setInput("");
    const newMsgs=[...msgs,{role:"user",content:t}]; setMsgs(newMsgs); setLoading(true); addXP(5);
      ...
      <div key={m.id} style={{ background:C.card, border:`1px solid ${open===m.id?m.color+"88":m.color+"33"}`, borderRadius:14, padding:20, marginBottom:16, cursor:"pointer" }} onClick={()=>setOpen(open===m.id?null:m.id)}>
        <div style={{ display:"flex", alignItems:"flex-start", gap:14 }}>
          ...
          <div style={{ flex:1 }}>
            ...
            <div style={{ fontSize:12, color:C.muted, marginBottom:8 }}>{m.description}</div>
    ...
    <aside style={{ width:220, background:C.surface, borderRight:`1px solid ${C.border}`, display:"flex", flexDirection:"column", flexShrink:0, zIndex:10 }}>
      <div style={{ padding:"20px 16px", borderBottom:`1px solid ${C.border}` }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          <div><div style={{ fontSize:13, fontWeight:600 }}>{user.name.split(" ")[0]}</div><div style={{ fontSize:10, color:C.muted, marginTop:1 }}>{user.role==="admin"?"Administrador":user.role==="teacher"?`Docente`:`Est. G${user.grade||""}`}</div></div>
        </div>
      </div>
