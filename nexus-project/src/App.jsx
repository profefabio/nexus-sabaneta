...
function LoginView({ onLogin, error }) {
  ...
  const handleSubmit = () => {
    if (mode === "teacher") {
      onLogin({ type:"teacher", email, password: pw });
    } else {
      onLogin({ type:"student", nombre: nombre.trim(), apellido: apellido.trim(), grado, grupo });
    }
  };
  ...
  return (
    <div style={{ display:"flex", height:"100vh", position:"relative", zIndex:5 }}>
      ...
      <div style={{ flex:1, background:`linear-gradient(135deg, #070d1a 0%, #0d1f3c 100%)`, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:40, borderRight:`1px solid ${C.border}` }}>
        ...
        <div style={{ fontSize:13, color:C.muted, letterSpacing:2, textAlign:"center", lineHeight:1.8 }}>
          Plataforma Educativa<br/>I.E. Sabaneta
        </div>
        <div style={{ marginTop:48, background:`${C.accent}10`, border:`1px solid ${C.border}`, borderRadius:14, padding:20, width:"100%", maxWidth:280 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:10, color:C.muted, marginTop:1 }}>Tecnología e Informática · Grados 7–11</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div style={{ width:"100%", maxWidth:420 }}>
          ...
          <div style={{ display:"flex", background:C.surface, borderRadius:14, padding:4, marginBottom:28, border:`1px solid ${C.border}` }}>
            {[["student","🎓","Soy Estudiante"],["teacher","📚","Soy Docente"]].map(([m,ic,lb])=>(
              <button key={m} onClick={()=>setMode(m)} style={{ flex:1, padding:"11px 8px", borderRadius:10, border:"none", cursor:"pointer", fontFamily:"inherit", fontWeight:700, fontSize:13, transition:"all .2s",
                ...
                boxShadow: mode===m ? `0 4px 14px ${C.accent}44` : "none",
              }}>{ic} {lb}</button>
            ))}
          </div>
          ...
          {mode === "student" && (
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              <div style={{ textAlign:"center", marginBottom:4 }}>
                ...
                <div style={{ fontSize:18, fontWeight:800, color:C.text }}>Ingreso Estudiantes</div>
                <div style={{ fontSize:12, color:C.muted, marginTop:4 }}>Escribe tu nombre completo y selecciona tu grado</div>
              </div>
              <div>
                <div style={lbl}>Nombres</div>
                <input style={inp} placeholder="Ej: Juan Carlos" value={nombre} onChange={e=>setNombre(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleSubmit()} />
              </div>
              <div>
                <div style={lbl}>Apellidos</div>
                <input style={inp} placeholder="Ej: Pérez García" value={apellido} onChange={e=>setApellido(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleSubmit()} />
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                <div>
                  <div style={lbl}>Grado</div>
              ...
              <button style={{ padding:"13px 20px", background:`linear-gradient(135deg,${C.accent3},#059669)`, border:"none", borderRadius:12, color:"#fff", fontWeight:700, fontSize:14, cursor:"pointer", boxShadow:`0 6px 20px rgba(16,217,138,.3)` }}
                onClick={handleSubmit} disabled={!nombre||!apellido||!grado||!grupo}>
                Entrar al aula NEXUS 🚀
              </button>
            </div>
          )}
          ...
          {mode === "teacher" && (
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              <div style={{ textAlign:"center", marginBottom:4 }}>
                <div style={{ fontSize:20, marginBottom:6 }}>📚</div>
                <div style={{ fontSize:18, fontWeight:800, color:C.text }}>Ingreso Docentes</div>
              ...
              <div>
                ...
                <input style={inp} type="email" placeholder="usuario@sabaneta.edu.co" value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleSubmit()} />
              </div>
              <div>
              ...
              {error && <div style={{ background:"#ff444422", border:"1px solid #ff444444", color:"#ff7777", padding:"10px 14px", borderRadius:8, fontSize:13 }}>{error}</div>}
              <button style={{ padding:"13px 20px", background:`linear-gradient(135deg,${C.accent},${C.accent2})`, border:"none", borderRadius:12, color:"#fff", fontWeight:700, fontSize:14, cursor:"pointer", boxShadow:`0 6px 20px rgba(0,200,255,.3)` }}
                onClick={handleSubmit} disabled={!email||!pw}>
                Ingresar al sistema ➤
              </button>
            </div>
          )}
