...
function NexusChat({ prompt, userName, compact, user, misionId, equipo, misionData, misionTitle }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:compact?400:"100%", background:C.card, border:`1px solid ${C.border}`, borderRadius:isMobile?0:16, overflow:"hidden", position:"relative" }}>

      {/* ── Header: nivel XP + nota ── */}
      <div style={{ display:"flex", alignItems:"center", gap:8, padding:"7px 14px", background:C.surface, borderBottom:`1px solid ${C.border}`, position:"relative", flexShrink:0 }}>
      ...
      {misionData && !compact && (
        <div style={{ padding:"5px 14px 7px", background:`${colReto}08`, borderBottom:`1px solid ${colReto}22`, flexShrink:0 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
            <span style={{ fontSize:10, color:colReto, fontWeight:700 }}>
              {retoCompleto ? "🏁 Reto completado — 10/10" : `⚡ Interacciones del reto: ${interactionCount}/${MAX_INT}`}
            </span>
            <span style={{ fontSize:10, color:C.muted, fontFamily:"'Orbitron',monospace" }}>
              {retoCompleto ? `Nota: ${xpToNota(xp).toFixed(1)}` : `${MAX_INT - interactionCount} restantes`}
            </span>
          </div>
      ...
      {/* ── Input ── */}
      <div style={{ display:"flex", gap:7, padding:isMobile?"10px 10px 14px":"11px 12px", borderTop:`1px solid ${C.border}`, background:C.surface, alignItems:"flex-end", flexShrink:0 }}>
        <textarea
